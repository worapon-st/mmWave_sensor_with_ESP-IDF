...
static uint8_t fifo_buf[24576] = {0};
static uint16_t rx1_buf[16384] = {0};
...
void spi_acquire_task(void* arg) {
    while(1) {
        ...
        extract_fifo(fifo_buf, fifo_size, rx1_buf, 1, rx1_buf, NULL, NULL);

        send_uart(1, rx1_buf, NULL, NULL, rx_size);
